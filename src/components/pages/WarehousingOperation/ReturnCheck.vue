<template>
    <div class="arrival-check">
        ...
        <div class="arrival-check-main">
            <div class="check-if">
                <div class="check-left">
                    <el-input clearable placeholder="请扫描单号" ref='input2' @focus='flag="2"' spellcheck ='false' v-model="orderNum" class="ordernum" @keydown.native='scanEnter'>
            ...
            <div class="result-box" v-if='inforShow'>
                <div class="check-result clear">
                    ...
                    <div class="list-1">
                        <span>退货类型&nbsp;:</span>
                        <i>{{getNameByMaId('rejected_type',infor.sourceType)}}</i>
                    </div>
                    <!-- <div class="list-1">
                        <span>质检方式&nbsp;:</span>
            ...
            <div class="arrival-head">
                <div class='head-name'>销退质检列表</div>
                <div class="head-handle">
                    ...
                    <!-- <div class="box-num handle-box" @click='boxShow=true'><i class="iconfont iconxianghao"></i>&nbsp;箱号</div> -->
                </div>
            </div>
                ...
                <List :tableData='tableData'>
                    ...
                    >
                        ...
                        <el-table-column
                            ...
                            <!-- <template slot-scope="scope">
                                <img :src="scope.row.pictureUrl" alt="" class='goods-img'>
                            </template> -->
                        </el-table-column>
                        <el-table-column
                            prop="sku"
                        ...
                        <el-table-column
                            ...
                            <template slot-scope="scope">
                                ...
                                <!-- <el-input clearable placeholder="输入良品数" @keyup.native="e=>scope.row.goodProductNumber=e.target.value.replace(/[^\d]/g,'')" v-model="scope.row.goodProductNumber" class='text-mid'></el-input> -->
                            </template>
                        </el-table-column>
                        <!-- <el-table-column
                        ...
                        <!-- <el-table-column
                            ...
                            >
                            <template slot-scope="scope">
                                <el-input clearable placeholder="输入备注" v-model="scope.row.remark" class='text-mid'></el-input>
                            </template>
                        </el-table-column> -->
                        <el-table-column
                            label="操作"
        ...
        <el-dialog
            ...
            <el-form :model="unusualForm" :rules="unusualRules" ref="unusualForm" label-width="100px" class="unusual-form">
                ...
                <el-form-item label="数量" prop="exceptionSkuCount">
                    <el-input @keyup.native="e=>unusualForm.exceptionSkuCount=e.target.value.replace(/[^\d]/g,'')" clearable v-model="unusualForm.exceptionSkuCount" placeholder="请输入数量" required></el-input>
                </el-form-item>
        ...
        <el-dialog
            ...
            <el-form :model="bindForm" label-width="80px" class="bind-form">
                <!-- 防止按enter键刷新页面  -->
                ...
                <el-form-item label="数量" v-if='activeName=="second"'>
...
export default {
    name: 'WarehousingOperation_ReturnCheck',
    data (){
        return {
            dialogVisible: true,
            printAlert: false,
    ...
    created (){
        this.getErrorList('rejected_type','qc_exception','receive_arrival_notice_quality_type');
    },
    activated (){
    ...
    methods: {
        async search (){
            ...
            let params2 = {
                sourceNumber: this.orderNum,
                warehouseCode: this.configSet.wareHouse.dataCode,
                assistants: JSON.stringify(assistants)
            }
            let res;
            if (this.numKind == 1){
                res = await this.$api.Warehousing.backBaseYDList(params1)
            } else{
                res = await this.$api.Warehousing.backBaseLYList(params2)
            }
            if (res.errorCode === '100200'){
                ...
                this.infor.sourceType = data.sellingBackType;
        ...
        addQcNum (data,qualityControlOrderNumber){    //QC完成
            ...
            if (typeof data.goodProductNumber == 'undefined'){
                this.$utils.commonUtils.tip('QC完成数量不能为空','error')
                return;
            }
            if (data.goodProductNumber.trim() == ''){
                this.$utils.commonUtils.tip('QC完成数量不能为空','error')
                return;
            }
            let params = {
                ...
                sellingBackId: data.sellingBackId,
                sku: data.sku,
        ...
        judeSkuLen (){
            ...
            if ( len == 0 ){
                ...
                if (isFloor){
                    if (this.tableList[0].data.length > 1){
                        flag = 2;
                    } else{
                        flag = 1;
                    }
        ...
        clickBind (){
            ...
            let jude = this.judeSkuLen();     //绑定容器前的判断
            if (jude == 4){
                this.$utils.commonUtils.tip('你没有勾选任何单据','error')
            } else if(jude == 3){
                 if (skuList.length){   //存在未封箱数量是为0的情况
